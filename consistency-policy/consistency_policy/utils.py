...
def reward_calc(policy, task, device, num_runs, max_episode_length):
    if "_low" in task:
        task = task.replace("_low", "")
    if "_fix" in task:
        task = task.replace("_fix", "")
        env = MoveCubeEnv(task=task, fix_task=True, render_mode=None, dt=0.005, n_substeps=2, image_width=480, image_height=270, image_obs=True)
    else:
    ...
    try:
        ...
        p_gain = np.array([15.0, 15.0, 9.0] * 3)
        d_gain = np.array([0.05, 0.1, 0.05] * 3) 
        reward_total = []
        print("Start reward runs")
        for run in range(num_runs):
            env.reset()
            reward = []
            for i in range(max_episode_length):
                camera_images = env.get_camera_images()
                ...
                obs["action"] = output
                delta = output - obs["robot"]["position"]
                action = p_gain * delta - d_gain * obs["robot"]["velocity"]
                _, _, _, _, _ = env.step(action, return_obs=False)
                obs_temp = env.get_observation(detail = False, state=True)
                delta_temp = output - obs_temp["robot"]["position"]
                action_temp = p_gain * delta_temp - d_gain * obs_temp["robot"]["velocity"]
                _, obs["rew"], _, _, obs["has_achieved"] = env.step(action_temp, return_obs=True)
                reward.append(obs["rew"])
                if obs["has_achieved"]:
                    break
            print(np.mean(reward))
            reward_total.append(np.mean(reward))
