...
from trifinger_mujoco_env import MoveCubeEnv as TaskMujocoEnv
import h5py
import copy
...
class TriFingerEnv(Arm_Env):
	def __init__(self,
		...
		self.p_gain = np.array([2.5, 2.5, 1.5] * 3) #np.array([15.0, 15.0, 9.0] * 3)
		self.d_gain = np.array([0.1, 0.2, 0.1] * 3) #np.array([0.05, 0.1, 0.05] * 3)
		...
		self.name="Trifinger_Sim"
		self.dt = 0.005

		# initialize env
		print("Initializing Environment")
		if self.task == "none":
			self.env = MujocoEnv(objects=self.objects, render_mode="rgb_array", dt=self.dt, n_substeps=2, image_width=480, image_height=270, image_obs=True) # if changed also change value of IMAGE_RECORD_RESOLUTION_SIM in constants.py
		else:
			self.env = TaskMujocoEnv(task=task, fix_task=fix_task, render_mode="rgb_array", dt=self.dt, n_substeps=2, image_width=480, image_height=270, image_obs=True)
		self.obs = self.reset()
		if self.marker:
			self.marker_thumb = self.env.add_marker(
				{
			...
			self.marker_index = self.env.add_marker(
				{
					"pos": np.array([0.0, 0.0, 0.1]),
			...
			self.marker_middle = self.env.add_marker(
				{
					"pos": np.array([0.0, 0.0, 0.1]),
	...
	def get_endeff_position(self):
		return self.env.get_tip_position()
	...
	def take_action(self):
		obs = self.env.get_observation(detail = False, state=True)
		...
		target_angle = self.env.get_angles_from_tip_pos(target_valid).reshape(9)
		obs["action"] = target_angle
		if self.marker:
			self.marker_thumb.update("pos", target_pos_retarget[0])
			self.marker_index.update("pos", target_pos_retarget[1])
			self.marker_middle.update("pos", target_pos_retarget[2])
		obs_temp = obs
		for i in range(int(1/(self.frequency*self.dt*2))):
			delta = target_angle - obs_temp["robot"]["position"]
			action = self.p_gain * delta - self.d_gain * obs_temp["robot"]["velocity"]  #(delta-self.prev_delta)/0.02
			if i==int(1/(self.frequency*self.dt*2))-1:
				_, obs["rew"], _, _, obs["has_achieved"] = self.env.step(action, return_obs=True)
			else:
				_, _, _, _, _ = self.env.step(action, return_obs=False)
			obs_temp = self.env.get_observation(detail = False, state=True)
		rgb = self.env.render()[:, :, ::-1].astype(np.uint8)
		return rgb, obs
	
	def get_recorder_file_name(self):
		items = os.listdir(os.path.join(self.storage_path, self.task))
		...
		while True:
			try:
				...
				if delay>0:
					time.sleep(delay)
				self.last_time = time.time()
				color_image, self.obs = self.take_action()
